...
// 2G4通信地址
//------------------------------------------------------------------------------------------		
const u8 RF2G4_ADDR_TX[5] = { 'M', 'K', 'G', 'O', 'D' } ;		
const u8 RF2G4_ADDR_RX[5] = { 'M', 'K', 'G', 'O', 'D' } ;		
//------------------------------------------------------------------------------------------

u8 RF2G4_Send_Data[32] = { 0 };		// 2.4G发射数据缓存数组
			 
u8 RF2G4_Receive_Data[32] = { 0 };	// 2.4G接收数据缓存数组
...
void RF2G4_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;		//PB11浮空输入 				// PB11输入  
	GPIO_Init(GPIOB, &GPIO_InitStructure);
						 

	SPI2_Init_JX();    		// 初始化SPI	 
	
	GPIO_ResetBits(GPIOB,GPIO_Pin_13);//SCK置低
	GPIO_SetBits(GPIOB,GPIO_Pin_10);//CSN置高
	GPIO_ResetBits(GPIOB,GPIO_Pin_12);//CE置低
	GPIO_ResetBits(GPIOB,GPIO_Pin_15);//MOSI置低
...
void RF2G4_Mode_NOACK(uint8_t Mode)
{
    ...
    //SI24R1_CE_Clr();
    // 设置地址宽度为5bytes
    RF2G4_Write_Reg(W_REGISTER + SETUP_AW, 0X03);
	if(Mode == RX_MODE)
	{
	// 接收使用接收通道0和接收地址
    RF2G4_Write_Cont(W_REGISTER + RX_ADDR_P0, (u8*)RF2G4_ADDR_TX, TX_ADR_WIDTH);
	// 使能接收通道0
    RF2G4_Write_Reg(W_REGISTER + EN_RXADDR, RX_ADDR_P0);
	// 设置接收通道0负载数据宽度
    RF2G4_Write_Reg(W_REGISTER + RX_PW_P0, 32);
	}
	else
	{
	// 写入发送地址
    RF2G4_Write_Cont(W_REGISTER + TX_ADDR, (u8*)RF2G4_ADDR_TX, TX_ADR_WIDTH);
	// 使能NOACK	
    RF2G4_Write_Reg(W_REGISTER + FEATURE, 0X01);
	}		
    // 选择射频通道40
    RF2G4_Write_Reg(W_REGISTER + RF_CH, 40);
    // 数据传输率2Mbps，发射功率7dBm
    RF2G4_Write_Reg(W_REGISTER + RF_SETUP, 0X0F);
	if(Mode == RX_MODE)
	// CRC使能，16位CRC校验，上电，接收模式
    RF2G4_Write_Reg(W_REGISTER + CONFIG, (0X0F)|0x01);
	else
	// CRC使能，16位CRC校验，上电，发射模式
    RF2G4_Write_Reg(W_REGISTER + CONFIG, (0X0F)&0xfe);
    // 拉高CE启动接收设备
    RF2G4_CE=1;
}
